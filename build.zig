...
pub fn build(b: *std.Build) void {
    ...
    const untar_mod = b.createModule(.{
        .target = target,
        .optimize = optimize,
    });
    untar_mod.addCSourceFile(.{ .file = upstream.path("examples/untar.c") });
    untar_mod.linkLibrary(lib);

    const untar = b.addExecutable(.{
        .name = "untar",
        .root_module = untar_mod,
    });

    b.installArtifact(untar);
}
